...
public class SeatGeekHelper {
    ...
    public static final String PARAM_RANGE = "&range=50mi";
    ...
    public static void getEvents(Context context, Double latitude, Double longitude, InspoActivitiesAdapter adapter) {
        ...
        List<ActivityObj> sgActivities = new ArrayList<>();
        ...
        String url = BASE_URL + latlong + PARAM_RANGE +  apiKey;
            ...
            public void onResponse(String response) {
                try {
                    ...
                    for (int i = 0; i < events.length(); i++) {
                        ...
                        sgActivities.add(activityObj);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
                Log.d(TAG, "done parsing " + sgActivities.size());
                adapter.addData(sgActivities);
            }
        };
